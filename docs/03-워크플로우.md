...
```
AI Plan 모드 (+ 선택적 /peach-gen-spec) → /peach-gen-db → /peach-agent-team
```
...
```
/peach-gen-spec → 새 세션 → AI Plan 모드 → /peach-gen-db → /peach-agent-team
```

- Spec 문서를 먼저 작성하고, 새 세션에서 Spec을 컨텍스트로 주입해 구현
- 세션 간 컨텍스트: docs/spec/ + git log + auto memory로 충분
